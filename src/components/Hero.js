...
const Hero = () => {
  ...
  const [buttonsRef, buttonsView] = useInView(options)
      ...
      <main className='bg-black text-white w-full h-screen relative flex justify-center'>
        ...
        <div className='z-10 max-w-[85rem] mx-0 lg:mx-10 h-full flex flex-col lg:flex-row items-center justify-center'>
          ...
          <div className='w-[85%] lg:w-[50%] flex flex-col gap-y-[0.9rem]'>
            {/* TITLE */}
            <motion.h1 variants={animateTitle} initial="initial" ref={titleRef} animate={titleView ? "animate" : ""} className='color-main font-sora font-[900] text-[38px] lg:text-[51px] leading-[1em] tracking-[-2px]'>Alucard: The Next Generation Anime Token</motion.h1>

            {/* DESCRIPTION */}
            <motion.p variants={animateDesc} initial="initial" ref={descRef} animate={descView ? "animate" : ""} className='color-secondary font-inter font-[100] text-[10px] lg:text-[14px] tracking-[3px] leading-[24px] lg:leading-[28px]'>ALUCARD IS AN ENIGMATIC AND FORMIDABLE ENTITY, EMBODYING BOTH VILLAINY AND HEROISM. HIS FORMIDABLE POWER, UNMATCHED REGENERATIVE ABILITIES, AND UNPREDICTABLE BEHAVIOUR MAKE HIM A TOKEN TO BE FEARED BY MANY.</motion.p>

            {/* BUTTONS */}
            <motion.div variants={animateButtons} initial="initial" ref={buttonsRef} animate={buttonsView ? "animate" : ""} className='flex flex-col w-full lg:w-auto lg:flex-row flex-grow-[1] font-inter gap-x-3 gap-y-3 pt-[28.4px]'>
